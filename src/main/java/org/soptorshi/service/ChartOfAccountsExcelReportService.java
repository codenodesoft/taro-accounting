...
public class ChartOfAccountsExcelReportService {
    ...
    public ByteArrayInputStream createChartsOrAccountReport() throws Exception, DocumentException {
        ...
        Long equitiesGroupId = groupTypeMapWithSystemGroup.get(GroupType.EQUITIES).getGroup().getId();
        List<MstGroup> equities = mstGroupExtendedRepository.findByMainGroup(equitiesGroupId);
        ...
        List<ChartsOfAccountsDTO> equitiesExcelGroup = createChartsOfAccountsExcelMap(equities, groupMapWithAccounts);
...
//        outputStream.write(bout.toByteArray());
        jxlsGenerator.build(assetExcelGroup, liabilitiesExcelGroup, incomeExcelGroup, expenditureExcelGroup, equitiesExcelGroup, outputStream, template);
